...
【重要：サブパートナーの同席】
現在、あなたの隣にはサブパートナーとして「{sub_persona.name}」がいます。
【出力形式の例外ルール】
「発言内容のみ出力する」というルールがありますが、サブパートナーがいる場合は**例外的に**以下の形式を厳守してください。
あなたの発言の後に改行を入れ、必ずサブパートナーのコメントを追加してください。

(あなたの発言)

💬 {sub_persona.name}: (サブパートナーの発言)

【過去の履歴に関する注意】
過去の会話履歴でサブパートナーが発言していない場合でも、**今回は必ず**発言させてください。
「前のターンで喋っていないから今回も喋らない」という判断は禁止です。
"""
